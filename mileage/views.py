...
from .models import CarBrand, CarModel, Comment, Profile, Review, SparePart, SparePartCategory, User
...
def get_car_models(request, car_id):
    ...
    car_models = (
        CarModel.objects.filter(brand_id=car_id)
        .order_by("model_name")
...
def get_spare_parts_category(request, category_id):
    ...
    all_spare_parts = (
        SparePart.objects.filter(category_id=category_id)
        .order_by("name", "brand")
        .annotate(cnt=Count("review"))
    )
...
def add_new_spare_part(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                SparePart.objects.get(
                    name__iexact=s_p_name, brand__iexact=s_p_brand, number__iexact=s_p_number
                )
...
def get_model_spare_parts_reviews(request, model_id, spare_part_id):
    ...
    car_brand = CarBrand.objects.get(pk=car_model.brand_id)
    spare_parts = (
        Review.objects.filter(car_model_id=model_id, spare_part_id=spare_part_id)
        .order_by("-mileage")
    ...
    similar_spare_parts = (
        Review.objects.filter(
            spare_part__name__icontains=spare_part.name,
            spare_part__category_id=spare_part.category.id,
            car_model__id=model_id,
        )
        .order_by("spare_part__name", "spare_part__brand", "spare_part__number")
        .distinct("spare_part__name", "spare_part__brand", "spare_part__number")
        .select_related("spare_part")
    )
...
def get_private_user_profile(request):
    ...
    user_reviews = (
        Review.objects.filter(owner_id=request.user.id)
        .order_by("spare_part", "spare_part__category_id")
        .select_related("spare_part")
    )
...
def get_public_user_profile(request, user_id):
    ...
    user_reviews = (
        Review.objects.filter(owner_id=user.id)
        .order_by("spare_part", "spare_part__category_id")
        .select_related("spare_part")
    )
...
def search(request):
    ...
    if request.method == "GET":
        ...
        if query:
            search_result = SparePart.objects.filter(
...
def get_spare_part(request, spare_part_id):
    ...
    spare_parts_reviews = (
        Review.objects.filter(spare_part_id=spare_part_id)
    ...
    cars = (
        spare_part.review_set.all()
        .order_by("car_brand__brand", "car_model__model_name")
        ...
        if request.user.is_authenticated:
            if len(comment_text) > 20:
                comment = Comment.objects.create(
                    user=request.user, review=review, comments_text=comment_text
                )
