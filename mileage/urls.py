...
urlpatterns = [
    ...
    path(
        "model_spare_part/<int:model_id>/<int:spare_part_id>/",
        get_model_spare_parts_reviews,
        name="model_spare_parts_reviews",
    ),
    path("get_models/<int:brand_id>/", get_chained_car_models, name="ajax_get_chained_car_models"),
    path(
        "spare_part_autocomplete/", SparePartAutocomplete.as_view(), name="spare_part_autocomplete"
    ),
