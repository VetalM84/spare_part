...
class UserLoginForm(AuthenticationForm):
    ...
    password = forms.CharField(
        label="Пароль", widget=forms.PasswordInput(attrs={"class": "uk-input"})
    )
    ...
    class Meta:
        ...
        widgets = {
            "spare_part": autocomplete.ModelSelect2(
                url="spare_part_autocomplete", attrs={"class": "uk-select"}
            ),
...
class UserEditForm(UserChangeForm):
    ...
    last_name = forms.CharField(
        label="Фамилия", widget=forms.TextInput(attrs={"class": "uk-input"})
    )
    username = forms.CharField(label="Логин", widget=forms.TextInput(attrs={"class": "uk-input"}))
    email = forms.EmailField(label="Email", widget=forms.EmailInput(attrs={"class": "uk-input"}))
    ...
    def clean_drive2_link(self):
        ...
        if not drive2_link.startswith("https://www.drive2.ru/users/"):
            raise ValidationError(
                "Ссылка должна выглядеть так: https://www.drive2.ru/users/имя_пользователя"
            )
        return drive2_link
